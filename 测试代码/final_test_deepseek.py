...
#        # 统计LLM结果使用的正确数量（原始统计方式）
#         llm_easy_correct = len(group[(group['question_level'] == 0) & (group['correct'] == 1)])
#         llm_middle_correct = len(group[(group['question_level'] == 1) & (group['correct'] == 1)])
#         llm_hard_correct = len(group[(group['question_level'] == 2) & (group['correct'] == 1)])
        
#         # 统计Statistic结果使用的正确数量（排除异常时间的题目）
...
#     output_prefix = "/home/lms/project/dify-backend/data/test_data/output2_"+f # 输出文件前缀
